...
class MentionBotDebugDM {
	 ...
	 */
	async initDatabase() {
		this.db = new sqlite3.Database('./mention_bot_debug_dm.db');
	...
	setupEventHandlers() {
		...
		this.client.on('messageCreate', async (message) => {
			...
			console.log(`   Channel Type: ${typeof message.channel.type} - "${message.channel.type}"`);
			...
			console.log(`   Is DM (type 3): ${message.channel.type === 3}`);
			console.log(`   Is DM (string DM): ${message.channel.type === 'DM'}`);
			console.log(`   Is DM (string GROUP_DM): ${message.channel.type === 'GROUP_DM'}`);
			...
			const isDM = message.channel.type === 1 || message.channel.type === 3 || message.channel.type === 'DM' || message.channel.type === 'GROUP_DM'; // DM channel types
			
			console.log(`🔍 Processing decision:`);
			console.log(`   Is mentioned: ${isMentioned}`);
			console.log(`   Is DM: ${isDM}`);
			console.log(`   Should process: ${isMentioned || isDM}`);
			
			if (isMentioned || isDM) {
				console.log(`✅ Processing message...`);
				await this.handleMention(message);
			} else {
				console.log(`❌ Not processing - not mentioned and not DM`);
			}
	...
	async handleMention(message) {
		try {
			...
			if (!question && message.attachments.size === 0) {
				console.log(`   No question or attachments, sending welcome message`);
				const response = isDM ? 
			...
			if (message.attachments.size > 0 && /remember\s+as\s+/i.test(message.content)) {
				console.log(`   Processing file upload with label`);
				if (!label) {
...
// Start the bot
const bot = new MentionBotDebugDM();
bot.start();
